...
export default new Command({
  ...
  execute: async (command, options, utils) => {
    ...
    const end = Date.now();
    const view = new View(global.__ConflictViewParser("message", null, global.__ConflictViewParser(Embed, {
      color: "#FFD45D",
      footer: {
        text: `Generated ${response.length} characters in ${fancyElapsedTime(end - start)}`,
        icon_url: 'https://cdn.discordapp.com/attachments/785702152374321163/1087419518873239613/Frame_1_4.png'
      }
    }, global.__ConflictViewParser("description", null, response))));
    console.log(view);
